...
numnets = 1000; % Number of iterations to run
...
% The parameter for the Dirichlet (and Beta) distribution from which the probabilities 
% are sampled
% Biased: 0.1
% Uniform: 1
dirichletparam = 1;
...
for n=1:numnets
    ...
    dcpd = betarnd(dirichletparam,dirichletparam,[1 4]);
    hcpd = betarnd(dirichletparam,dirichletparam,[1 4]);
    vprior(1,:) = dirichletrnd([dirichletparam dirichletparam dirichletparam dirichletparam]);
    vprior(2,:) = dirichletrnd([dirichletparam dirichletparam dirichletparam dirichletparam]);
    ...
    for p=1:2
        bnet.CPD{V} = tabular_CPD(bnet, V, [vprior(p,:)]);
